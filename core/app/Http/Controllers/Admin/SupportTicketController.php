...
class SupportTicketController extends Controller
{
    ...
    {
        $pageTitle = 'Pending Tickets';
        $items = SupportTicket::where('agent_id', '!=', 0)->orderBy('id', 'desc')->with('agent')->paginate(getPaginate());
        return view('admin.support.tickets', compact('items', 'pageTitle'));
    }
    public function userTicket()
    {
        $pageTitle = 'Pending Tickets';
        $items = SupportTicket::where('user_id', '!=', 0)->orderBy('id', 'desc')->with('agent')->paginate(getPaginate());
        return view('admin.support.tickets', compact('items', 'pageTitle'));
    }
    public function guestTicket()
    {
        $pageTitle = 'Pending Tickets';
        $items = SupportTicket::where('user_id', 0)->where('agent_id', 0)->orderBy('id', 'desc')->with('agent')->paginate(getPaginate());
        return view('admin.support.tickets', compact('items', 'pageTitle'));
    }
