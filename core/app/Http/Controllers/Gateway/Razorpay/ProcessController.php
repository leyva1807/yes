...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['name'] = userGuard()['user']->username;
        $val['description'] = "Payment By Razorpay";
        $val['image'] = getImage(getFilePath('logoIcon') .'/logo.png');
        $val['prefill.name'] = userGuard()['user']->firstname . ' ' . userGuard()['user']->lastname;
        $val['prefill.email'] = userGuard()['user']->email;
        $val['prefill.contact'] = userGuard()['user']->mobile;
