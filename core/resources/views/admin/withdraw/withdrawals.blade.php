    ...
    <div class="row justify-content-center">
        @if (request()->routeIs('admin.withdraw.log') || request()->routeIs('admin.withdraw.method'))
            <div class="col-xl-4 col-sm-6 mb-30">
                <x-widget bg="success" color="white" link="{{ route('admin.withdraw.approved') }}" style="4" title="Approved Withdrawals" value="{{ __($general->cur_sym) }}{{ showAmount($successful) }}" />
            </div>
            <div class="col-xl-4 col-sm-6 mb-30">
                <x-widget bg="6" color="white" link="{{ route('admin.withdraw.pending') }}" style="4" title="Pending Withdrawals" value="{{ __($general->cur_sym) }}{{ showAmount($pending) }}" />
            </div>
            <div class="col-xl-4 col-sm-6 mb-30">
                <x-widget bg="pink" color="white" link="{{ route('admin.withdraw.rejected') }}" style="4" title="Rejected Withdrawals" value="{{ __($general->cur_sym) }}{{ showAmount($rejected) }}" />
            </div>
        @endif
        <div class="col-lg-12">
                            ...
                            <tbody>
                                @forelse($withdrawals as $withdraw)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="fw-bold">{{ $withdraw->agent->fullname }}</span>
                                            <br>
                                            <span class="small"> <a href="{{ appendQuery('search', @$withdraw->agent->username) }}"><span>@</span>{{ @$withdraw->agent->username }}</a> </span>
                                        </td>
