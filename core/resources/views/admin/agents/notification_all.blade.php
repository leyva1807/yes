            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mt-3">
                        ...
                        <p>@lang('Email sent') <span class="sent">0</span> @lang('agents out of') {{ $agents }} @lang('agents')</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        (function($) {
            ...
            function postMail(form, skip) {
                ...
                $.post("{{ route('admin.agents.notification.all.send') }}", {
                    "subject": subject,
                    "_token": _token,
                    "skip": skip,
                    "message": message
                    ...
                    if (response.error) {
                        ...
                        var rest = {{ $agents }} - response.total_sent;
                        var sentPercent = response.total_sent / {{ $agents }} * 100;
                        if (sentPercent > 100) {
                            sentPercent = 100;
                        }
                        ...
                        if (rest == 0) {
                            setTimeout(() => {
                                ...
                                notify('success', 'Mail sent to all agents successfully')
                            }, 3000);
                            return false;
                        }
